...
export const createBooking = async (req, res) => {
    ...
    const { room_id,type, num_of_guests, check_in_date, check_out_date, payment_status } = req.body;
    ...
    if (num_of_guests > room.capacity) {
        return res.status(400).json({ message: `${getMessage("capacity", lang)}${room.capacity}` });
    }
    ...
    if (specialPricingList.length) {
        ...
        const weeklyPricing = await RoomPricing.findAll({ where: { room_id } });
